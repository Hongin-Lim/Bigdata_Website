def DOCKER_IMAGE_NAME = "lhi90/verymarket-bigdata"           // 생성하는 Docker image 이름
            ...
            container('kubectl') {
                withCredentials([usernamePassword(
                    ...
                    passwordVariable: 'PASSWORD')]) {
                        ...
                        sh "echo ${DATE}"
                        sh "sed -i.bak 's#DATE_STRING#${DATE}#' ./Bigverymarket-deployment.yaml"

                        /* yaml파일로 배포를 수행한다 */
                        sh "kubectl apply -f ./Bigverymarket-deployment.yaml -n ${NAMESPACE}"
                        sh "kubectl apply -f ./Bigverymarket-service.yaml -n ${NAMESPACE}"
                }
            }
        }
    }
